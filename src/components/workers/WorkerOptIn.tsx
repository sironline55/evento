...
export function WorkerOptIn({ attendeeId, attendeeName, attendeePhone = '', attendeeEmail }: Props) {
  ...
  async function submit() {
    ...
    setLoading(true)
    try {
      await WorkerService.optInFromAttendee(
        attendeeId,
        {
  ...
  return (
    <div style={{ border: '1px solid #d1fae5', borderRadius: 16, overflow: 'hidden', marginTop: 16 }}>
      <div
        style={{ background: '#f0fdf4', padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}
            ...
            {(['male','female'] as const).map(g => (
              <button
                ...
                style={{
                  ...
                  borderColor: form.gender === g ? '#2B6E64' : '#e5e7eb',
                  background: form.gender === g ? '#2B6E64' : '#fff',
                  color: form.gender === g ? '#fff' : '#374151',
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {QUICK_SKILLS.map(s => (
                <button
                  ...
                  style={{
                    padding: '6px 12px', borderRadius: 20, border: 'none', fontSize: 12, cursor: 'pointer',
                    background: form.skills.includes(s) ? '#2B6E64' : '#f3f4f6',
                    color: form.skills.includes(s) ? '#fff' : '#374151'
                  }}
                >
                  {s}
                </button>
