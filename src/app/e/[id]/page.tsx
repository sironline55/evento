...
export default async function PublicEventPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, direction:'rtl', fontFamily:'system-ui,sans-serif' }}>
      ...
      {ev.cover_image ? (
        ...
        <div style={{ height:200, background:`linear-gradient(135deg,${C.navy},#3D1A78)` }}/>
      )}
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'0 16px' }}>
        ...
        <div style={{ background:C.card, borderRadius:12, border:`1px solid ${C.border}`, marginTop:-40, position:'relative', zIndex:10, overflow:'hidden', marginBottom:16 }}>
          <div style={{ padding:'24px 24px 20px' }}>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:12, flexWrap:'wrap' }}>
              {ev.category && (
                <span style={{ fontSize:12, padding:'4px 10px', background:'#EDE9F7', color:'#7B4FBF', borderRadius:12, fontWeight:600 }}>{ev.category}</span>
              )}
          ...
          <div style={{ borderTop:`1px solid ${C.border}`, padding:'16px 24px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            ...
            {ev.location_type !== 'tba' && (
              <div style={{ display:'flex', gap:10 }}>
                <span style={{ fontSize:20, flexShrink:0 }}>{ev.location_type==='online'?'💻':'📍'}</span>
                <div>
                  <p style={{ fontSize:11, color:C.muted, margin:0 }}>{ev.location_type==='online'?'عبر الإنترنت':'الموقع'}</p>
                  <p style={{ fontSize:13, fontWeight:600, color:C.text, margin:'2px 0 0' }}>{ev.location || 'يُحدد لاحقاً'}</p>
                </div>
              </div>
            )}
            {ev.capacity && (
            ...
            {ev.show_attendee_count !== false && regCount! > 0 && (
              <div style={{ display:'flex', gap:10 }}>
                <span style={{ fontSize:20, flexShrink:0 }}>👥</span>
                <div>
        ...
        {lineup.length > 0 && (
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:24, marginBottom:16 }}>
            <h2 style={{ fontSize:16, fontWeight:700, color:C.navy, margin:'0 0 16px' }}>🎤 المتحدثون</h2>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              {lineup.map((p: any, i: number) => (
                <div key={i} style={{ display:'flex', gap:12, padding:12, background:'#F8F7FA', borderRadius:8 }}>
                  <div style={{ width:44, height:44, borderRadius:'50%', background:`linear-gradient(135deg,${C.navy},#7B4FBF)`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:'#fff', fontWeight:700, fontSize:16 }}>
                    {p.name?.charAt(0)}
                  </div>
                  <div style={{ minWidth:0 }}>
                    <p style={{ fontSize:13, fontWeight:700, color:C.navy, margin:0 }}>{p.name}</p>
                    <p style={{ fontSize:11, color:C.muted, margin:'2px 0 0' }}>{p.role}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {isFull ? (
              ...
              <>
                <p style={{ color:'rgba(255,255,255,0.7)', fontSize:13, margin:'0 0 8px' }}>
                  {spotsLeft ? `${spotsLeft} مقعد متبقي فقط` : 'التسجيل مجاني ومفتوح'}
                </p>
                <Link href={`/r/${ev.id}`} style={{
