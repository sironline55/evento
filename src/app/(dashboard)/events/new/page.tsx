...
const CATEGORIES: Record<string,string[]> = {
  ...
  'رياضة ونشاط':   ['ماراثون','مباراة','تمرين جماعي'],
  'أعمال وتجارة':  ['ملتقى أعمال','فرص استثمار','تواصل مهني','إطلاق منتج'],
  'تكنولوجيا':      ['هاكاثون','مؤتمر تقني','ورشة برمجة','ذكاء اصطناعي'],
  'تعليم ودورات':  ['دورة تدريبية','محاضرة','تطوير مهني'],
  'طعام وشراب':    ['مهرجان طعام','تذوق','ورشة طبخ'],
...
type Ticket = { id:number; name:string; type:'free'|'paid'; price:string; qty:string; desc:string; saleStart:string; saleEnd:string }
...
function EventPreview({ title, subtitle, category, subcategory, startDate, startTime, endDate, endTime,
  ...
  return (
    <div style={{ width:isMobile?320:'100%', margin:'0 auto', background:'#F0F3F7',
      borderRadius:isMobile?24:12, overflow:'hidden',
      ...
      {coverPreview
        ? <img src={coverPreview} alt="cover" style={{width:'100%',height:isMobile?160:220,objectFit:'cover',display:'block'}}/>
        : <div style={{height:isMobile?160:220,background:'linear-gradient(135deg,#E8E0F0,#D0C8E8)',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontSize:40}}>🖼️</span></div>
      }
      <div style={{padding:'16px 16px 0',background:C.card}}>
        <h2 style={{fontSize:isMobile?18:24,fontWeight:800,color:C.navy,margin:'0 0 8px',lineHeight:1.3}}>{title||'عنوان الفعالية'}</h2>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:14}}>
          ...
          {eventUrl&&locationType==='online' && <div style={{display:'flex',gap:6,alignItems:'center'}}><span>🔗</span><span style={{fontSize:12,color:'#3D7FE8'}}>رابط البث متاح للمسجلين</span></div>}
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginBottom:12}}>
          ...
          <p style={{fontSize:12,color:C.text,lineHeight:1.7,margin:0}}>{description||'لا يوجد وصف بعد...'}</p>
        </div>
        {good.length>0 && (
          <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginBottom:12}}>
            ...
            <div style={{background:'#F8F7FA',borderRadius:8,padding:'12px',border:`1px solid ${C.border}`}}>
              ...
              {good.map((g,i)=>(
                <div key={i} style={{display:'flex',gap:6,marginBottom:5}}><span style={{fontSize:13}}>{g.icon}</span><span style={{fontSize:12,color:C.text}}>{g.text}</span></div>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,paddingBottom:70}}>
          ...
          <div style={{background:'#F8F7FA',borderRadius:10,padding:'12px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
              <button style={{padding:'8px',border:`1px solid ${C.border}`,borderRadius:6,background:C.card,fontWeight:600,fontSize:12,cursor:'pointer'}}>تواصل</button>
              <button style={{padding:'8px',border:'none',borderRadius:6,background:C.orange,color:'#fff',fontWeight:700,fontSize:12,cursor:'pointer'}}>متابعة</button>
            </div>
          </div>
        </div>
      </div>
      <div style={{position:'sticky',bottom:0,background:C.card,borderTop:`1px solid ${C.border}`,padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <button disabled style={{padding:'8px 16px',border:'none',borderRadius:6,background:'#D9D9D9',color:'#9E9E9E',fontWeight:700,fontSize:12,cursor:'not-allowed'}}>احجز تذكرة</button>
      </div>
    </div>
  )
}
...
export default function NewEventPage() {
  ...
  const card: React.CSSProperties = {
    background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:'20px', marginBottom:12,
  }
  ...
  return (
    <div style={{minHeight:'100vh',background:C.bg,direction:'rtl'}}>
      ...
      <div style={{display:'flex',gap:22,maxWidth:showPreview?1500:820,margin:'0 auto',padding:'20px 18px 100px',alignItems:'flex-start'}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          {step===1 && (
            <>
              ...
              <div style={{...card,padding:0,overflow:'hidden'}}>
                ...
                {coverPreview ? (
                  <div style={{position:'relative'}}>
                    ...
                    <div style={{position:'absolute',top:10,left:10,display:'flex',gap:8}}>
                      <button onClick={()=>fileRef.current?.click()} style={{background:'rgba(0,0,0,0.7)',color:'#fff',border:'none',borderRadius:6,padding:'6px 12px',fontSize:12,cursor:'pointer',fontWeight:600}}>
                        {imageUploading?'⏳ جاري الرفع...':'✏️ تغيير'}
                      </button>
                      <button onClick={()=>{setCoverPreview(null);setCoverUrl('')}} style={{background:'rgba(220,38,38,0.8)',color:'#fff',border:'none',borderRadius:6,padding:'6px 10px',fontSize:12,cursor:'pointer'}}>✕</button>
                    </div>
                    {imageUploading && <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.3)',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{color:'#fff',fontWeight:700}}>⏳ جاري الرفع...</span></div>}
                  </div>
                ) : (
                  <div onClick={()=>fileRef.current?.click()} style={{padding:'32px 20px',textAlign:'center',cursor:'pointer',background:'#F3F0F8',border:'2px dashed #B4A7D6'}}>
                    ...
                    <p style={{fontSize:11,color:C.muted,margin:'0 0 8px'}}>PNG، JPG أو GIF — حد أقصى 10MB — يُوصى بـ 2160×1080</p>
                    <p style={{fontSize:11,color:C.green,margin:0,fontWeight:600}}>💡 الفعاليات التي تحتوي على صور تحصل على زيارات أكثر بـ 30%</p>
                  </div>
                )}
              </div>
              ...
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 3px'}}>تفاصيل الفعالية</p>
                <p style={{fontSize:12,color:C.muted,margin:'0 0 14px'}}>اسم واضح يخبر الناس ما سيشاهدون</p>
                <div style={{marginBottom:12}}>
                  <label style={lbl}>اسم الفعالية *</label>
                  <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="اسم قصير وواضح" maxLength={75}
                ...
                <div style={{marginBottom:12}}>
                  <div style={{display:'flex',justifyContent:'space-between'}}>
                    <label style={lbl}>الوصف المختصر <span style={{color:C.muted,fontWeight:400}}>(يظهر في نتائج البحث)</span></label>
                  ...
                  <textarea value={subtitle} onChange={e=>setSubtitle(e.target.value.slice(0,140))}
                    placeholder="جملة قصيرة تلخص فعالتك — 140 حرف كحد أقصى" rows={2}
              ...
              <div style={card}>
                ...
                {isRecurring && (
                  <div style={{marginBottom:14,padding:12,background:'#FEF0ED',borderRadius:8,border:'1px solid #F5C0B0'}}>
                    <label style={{...lbl,marginBottom:8}}>تكرار الفعالية</label>
                    <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                      {RECURRENCE.map(({val,label})=>(
                        <button key={val} onClick={()=>setRecurrence(val)} style={{
                          padding:'6px 14px',borderRadius:50,fontWeight:600,fontSize:12,cursor:'pointer',
                          border:`1px solid ${recurrence===val?C.orange:C.border}`,
              ...
              {/* Location */}
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 14px'}}>الموقع</p>
                ...
                {locationType==='venue' && (
                  <>
                    <div style={{marginBottom:10}}>
                      ...
                      <input value={location} onChange={e=>setLocation(e.target.value)} placeholder="مثال: قاعة الملك فهد، الرياض" style={inp}/>
                    </div>
                    <div style={{marginBottom:12}}>
                      <label style={lbl}>تفاصيل إضافية <span style={{color:C.muted,fontWeight:400}}>(اختياري)</span></label>
                      <input value={locationDetails} onChange={e=>setLocDetails(e.target.value)} placeholder="الطابق الثاني، قاعة B، بالقرب من..." style={inp}/>
                    </div>
                  </>
                )}
                {locationType==='online' && (
                  <div style={{marginBottom:12}}>
                    ...
                    <input value={eventUrl} onChange={e=>setEventUrl(e.target.value)} placeholder="https://zoom.us/j/..." type="url" style={inp}/>
                    <p style={{fontSize:11,color:C.muted,marginTop:4}}>🔒 يظهر للمسجلين فقط بعد التسجيل</p>
                  </div>
                )}
                <div>
                  <label style={lbl}>الطاقة الاستيعابية</label>
                  <div style={{display:'flex',gap:10,alignItems:'center'}}>
                    <input type="number" value={capacity} onChange={e=>setCapacity(e.target.value)} placeholder="عدد الزوار" style={{...inp,maxWidth:160}}/>
                    <span style={{fontSize:11,color:C.muted}}>اتركها فارغة للسماح بعدد غير محدود</span>
                  </div>
                </div>
              </div>
              ...
              {/* Description */}
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 4px'}}>نبذة عن الفعالية</p>
                <p style={{fontSize:12,color:C.muted,margin:'0 0 12px'}}>أخبر الزوار بما يمكن توقعه</p>
                <textarea value={description} onChange={e=>setDesc(e.target.value)}
                  placeholder="اكتب هنا كل ما تريد أن يعرفه الزائر..." rows={6}
                  style={{...inp,resize:'vertical',lineHeight:1.6}}/>
              </div>

              {/* Good to Know */}
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 4px'}}>معلومات مهمة</p>
              ...
              {/* Agenda */}
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 4px'}}>الجدول الزمني</p>
                <p style={{fontSize:12,color:C.muted,margin:'0 0 14px'}}>أضف برنامج الفعالية جلسة بجلسة</p>
                {agenda.map((item,i)=>(
              ...
              {/* Lineup */}
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 4px'}}>المتحدثون والفنانون</p>
                <p style={{fontSize:12,color:C.muted,margin:'0 0 14px'}}>أضف المشاركين البارزين في فعالتك</p>
                {lineup.map((item,i)=>(
              ...
              {/* FAQ */}
              <div style={card}>
                <p style={{fontSize:15,fontWeight:700,color:C.navy,margin:'0 0 4px'}}>الأسئلة الشائعة</p>
                {showFaqTip && (
              ...
              {tickets.map((tk,i)=>(
                <div key={tk.id} style={card}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:14}}>
                    ...
                    {tickets.length>1 && <button onClick={()=>removeTicket(tk.id)} style={{background:'none',border:'none',cursor:'pointer',color:'#DC2626',fontSize:12,fontWeight:600}}>حذف</button>}
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:10,marginBottom:12}}>
                    <div>
                      <label style={lbl}>اسم التذكرة</label>
                      <input value={tk.name} onChange={e=>updateTicket(tk.id,'name',e.target.value)} placeholder="مثال: عامة، VIP..." style={inp}/>
                    </div>
                    <div>
                  ...
                  <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12}}>
                    ...
                    <p style={{fontSize:11,color:C.muted,marginTop:6}}>اتركهما فارغتين للبيع حتى يوم الفعالية</p>
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              <div style={{...card,border:`2px solid ${C.border}`}}>
                ...
                {[
                  ...
                  ['🔄 التكرار',  isRecurring?RECURRENCE.find(r=>r.val===recurrence)?.label||recurrence:'فعالية منفردة'],
              ...
              <div style={card}>
                ...
                <div style={{display:'flex',gap:10}}>
                  {[['public','🌐 عامة','تظهر في نتائج البحث'],['private','🔒 خاصة','يصل إليها من يملك الرابط']].map(([v,l,d])=>(
                    <div key={v} onClick={()=>setVisibility(v as any)} style={{flex:1,padding:14,borderRadius:10,cursor:'pointer',border:`2px solid ${visibility===v?C.orange:C.border}`,background:visibility===v?'#FEF0ED':C.card}}>
              ...
              <div style={card}>
                ...
                <div style={{display:'flex',gap:10}}>
                  {[['draft','📝 مسودة','احفظ للمراجعة لاحقاً'],['published','🚀 نشر الآن','الفعالية متاحة فوراً']].map(([v,l,d])=>(
                    <div key={v} onClick={()=>setStatus(v as any)} style={{flex:1,padding:14,borderRadius:10,cursor:'pointer',border:`2px solid ${status===v?C.green:C.border}`,background:status===v?'#EAF7E0':C.card}}>
        ...
        {showPreview && (
          <div style={{width:360,flexShrink:0,position:'sticky',top:100}}>
            <div style={{display:'flex',justifyContent:'center',gap:8,marginBottom:12}}>
              {([['mobile','موبايل'],['desktop','ديسكتوب']] as const).map(([m,label])=>(
                <button key={m} onClick={()=>setPreviewMode(m)} style={{
                  display:'flex',alignItems:'center',gap:5,padding:'7px 16px',borderRadius:8,cursor:'pointer',fontWeight:600,fontSize:12,
            ...
            <p style={{textAlign:'center',fontSize:10,color:C.muted,marginBottom:10,fontWeight:600,letterSpacing:'0.06em'}}>معاينة مباشرة</p>
            <EventPreview
      ...
      {/* Bottom nav */}
      <div style={{position:'fixed',bottom:0,left:0,right:0,background:C.card,borderTop:`1px solid ${C.border}`,padding:'11px 24px',display:'flex',justifyContent:'space-between',alignItems:'center',zIndex:20,direction:'rtl'}}>
        <button onClick={()=>setStep(s=>Math.max(1,s-1))} disabled={step===1} style={{padding:'9px 20px',border:`1px solid ${C.border}`,borderRadius:6,background:C.card,cursor:step===1?'not-allowed':'pointer',color:step===1?C.muted:C.text,fontWeight:600,fontSize:13,opacity:step===1?0.4:1}}>← السابق</button>
