    ...
    <div style={{ padding: 24, direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <Link href={'/scanner'} style={{ padding: '10px 18px', background: '#2B6E64', color: '#fff', borderRadius: 10, textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>📷 فتح الماسح</Link>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي المسجلين', value: count, color: '#2B6E64' },
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 24, border: '1px solid #f0f0f0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <Link href={'/events/' + id + '/attendees'} style={{ fontSize: 13, color: '#2B6E64', textDecoration: 'none', fontWeight: 500 }}>عرض الكل ←</Link>
        </div>
        {registrations.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '30px 0', color: '#999' }}>
            <p>لا يوجد مسجلون بعد</p>
            <button onClick={copyRegLink} style={{ background: '#2B6E64', color: '#fff', padding: '10px 20px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, marginTop: 8 }}>
              📋 مشاركة رابط التسجيل
            </button>
          </div>
        ) : (
